...
class VoyagerEmailHistoryController extends BaseVoyagerBaseController {
    ...
    protected function browse(Request $request) {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('email_id', function($query) {
                    if($query->email instanceof Email) {
                        return $query->email->name;
                    } else {
                ...
                ->editColumn('status', function($query) {
                    return ($query->status) ?
                        DataTableHelper::getStatusLabel('Sent', true) :
                        DataTableHelper::getStatusLabel('Failed', false);
                })
                ->editColumn('is_processed', function($query) {
                    return ($query->is_processed) ?
                        DataTableHelper::getStatusLabel('Processed', true) :
                        DataTableHelper::getStatusLabel('Pending', false);
                })
                ...
                ->rawColumns(['action', 'id', 'avatar', 'status', 'is_processed'])
                ->make(true);
        } else {
            abort(404);
        }
    ...
    public function store(Request $request)
    {
        ...
        $data = Campaign::create([
            ...
            'showcase_id' => $request->get('showcase_id'),
